...
#
# Adapted from allenai/open-instruct. The MetaMathQA transforms
# (sft_metamathqa_tokenize_and_truncate_v1, sft_metamathqa_filter_v1) are ours.
...
def get_tokenizer_tulu_v2_2(tc: "TokenizerConfig"):
    ...
    if not hasattr(tokenizer, "chat_template") and tc.chat_template_name in CHAT_TEMPLATES:
        tokenizer.chat_template = CHAT_TEMPLATES[tc.chat_template_name]
...
def sft_magicoder_tokenize_and_truncate_v1(row: Dict[str, Any], tokenizer: PreTrainedTokenizer, max_seq_length: int):
    """
    Convert Magicoder format to messages and then tokenize and truncate.
    Combines conversion and tokenization in one step.
    """
    # 1) Build messages
    messages = [
...
def sft_magicoder_filter_v1(row: Dict[str, Any], tokenizer: PreTrainedTokenizer):
    """Filter function for Magicoder tokenized data"""
    return any(x != -100 for x in row["labels"])

def sft_metamathqa_tokenize_and_truncate_v1(row: Dict[str, Any], tokenizer: PreTrainedTokenizer, max_seq_length: int):
    """
    Convert MetaMathQA format to messages and then tokenize and truncate.
    Combines conversion and tokenization in one step.
    """
    # 1) Build messages
    messages = [
...
def sft_metamathqa_filter_v1(row: Dict[str, Any], tokenizer: PreTrainedTokenizer):
    """Filter function for MetaMathQA tokenized data"""
    return any(x != -100 for x in row["labels"])
...
TRANSFORM_FNS = {
    ...
    "sft_metamathqa_tokenize_and_truncate_v1": (sft_metamathqa_tokenize_and_truncate_v1, "map"),
    "sft_metamathqa_filter_v1": (sft_metamathqa_filter_v1, "filter"),
    "sft_magicoder_tokenize_and_truncate_v1": (sft_magicoder_tokenize_and_truncate_v1, "map"),
    "sft_magicoder_filter_v1": (sft_magicoder_filter_v1, "filter"),
